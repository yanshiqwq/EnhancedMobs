...
import cn.yanshiqwq.enhanced_mobs.script.DslBuilder.TypeBuilder.TaskId
...
pack("extend") {
    ...
    type("spider_cobweb", "vanilla.spider") { mob ->
        val taskPeriod = (80 - mob.multiplier * 10).toLong().coerceIn(50L..100L)
        itemTask(
            distance = 5.0,
    ...
    type("zombie_shield", "vanilla.zombie") { mob ->
        ...
        listener<EntityDamageByEntityEvent> {
            ...
            if (entity.persistentDataContainer.getOrDefault(shieldKey, PersistentDataType.BYTE, 0) != 1.toByte()) {
                val onShieldChance = (0.35 * (mob.multiplier + 1)).coerceIn(0.0, 0.85)
    ...
    type("zombie_cobweb", "vanilla.zombie") { mob ->
        val taskPeriod = (100 - mob.multiplier * 10).toLong().coerceIn(60L..120L)
        itemTask(
            distance = 5.0,
            before = ItemStack(Material.COBWEB),
            period = taskPeriod
